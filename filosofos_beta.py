...
class filosofo(threading.Thread):
    ...
    def __init__(self):
        ...
        print(" FILOSOFO {0} - SE SIENTA EN LA MESA".format(self.id))

    def pensar(self):
        time.sleep(random.randint(0,10)) #CADA FILOSOFO SE TOMA DISTINTO TIEMPO PARA PENSAR, ALEATORIO
        print(" FILOSOFO {} PENSANDO".format(self.id))

    def derecha(self,i):
        return (i-1)%FILOSOFOS #BUSCAMOS EL INDICE DE LA DERECHA

    def izquierda(self,i):
        return(i+1)%FILOSOFOS #BUSCAMOS EL INDICE DE LA IZQUIERDA
    ...
    def verificar(self,i):
        if filosofo.estado[i] == 'HAMBRIENTO' and filosofo.estado[self.izquierda(i)] != 'COMIENDO' and filosofo.estado[self.derecha(i)] != 'COMIENDO':
            filosofo.estado[i]='COMIENDO' #CAMBIA SU ESTADO A COMIENDO
            filosofo.tenedores[i].release()  #SI SUS VECINOS NO ESTAN COMIENDO AUMENTA EL SEMAFORO DEL TENEDOR
